...
<%
	...
	String saveFilename = request.getParameter("sName"); //저장파일명
	String originalFilename = request.getParameter("oName"); //원본파일명
	...
	String client = request.getHeader("User-Agent"); //웹브라우저 종류 판단
	if(client.indexOf("WOW64") == -1) {
		originalFilename = new String(originalFilename.getBytes("UTF-8"), "iso-8859-1");
		//리눅스, 유닉스(chrome, firefox, 모질라)
	}else{
	...
	// 파일 다운로드용 응답헤더 설정
	response.reset(); //초기화
	response.setContentType("application/octet-stream"); //8bit씩 처리(=1byte)
	...
	outputStream.close();
		...
		JSFunction.alertBack("다운로드 중 예외발생, 관리자를 호출하세요.", out);
	}
